...
class OpenAIProvider(LLMInterface):
    ...
    def generate_text(self, prompt: str, chat_history: list=[], max_output_tokens: int=None,
        ...
        if not resposne or not resposne.choices or len(resposne.choices) == 0 or not resposne.choices[0].message:
            self.logger.error("No response data returned from OpenAI API.")
            return None
